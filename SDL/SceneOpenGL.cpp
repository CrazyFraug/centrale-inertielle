...
void SceneOpenGL::bouclePrincipaleSimu(Mobile &gant, Traitement **simu)
{
	...
	while (!terminer)
	{
		...
		vect3D anglePlus = simu[0]->calculerAngle_deg(1);
		vect3D acce = simu[1]->calculerAngle_deg(1);
		vect3D mnet = simu[2]->calculerAngle_deg(1);
		vect3D orie = simu[3]->calculerAngle_deg(1);
